...
def latlon_to_pixel(lat, lon, image_width, image_height, bbox):
    ...
    y_pixel = image_height - y_pixel  # Invert y-axis for image coordinates
...
async def main():
    for image_name in image_list:
        ...
        # Extract coordinates from the image name
        ...
        # Check for turning spaces within this image's bounds
        for _, row in gdf.iterrows():

            # Replace gpd.box with shapely's box
            image_bounds = box(img_min_lon, img_min_lat, img_max_lon, img_max_lat)

            # Check if the turning space polygon intersects with the image bounds
            if row.geometry.intersects(image_bounds) and not row.geometry.within(image_bounds):
                ...
                cv2.fillPoly(mask, points, 255)  # Fill turning point polygon as white
