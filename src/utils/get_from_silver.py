...
def get_file_list_from_silver(spark, silver_table: str, mode: str = "train") -> list[str]:
    """
...
def get_split_from_silver(spark, silver_table: str, val_size=0.2, holdout_size=5, seed=42):
    
    df = spark.read.format("delta").load(silver_table).filter(
        "image_status = 'DOWNLOADED' AND dom_status = 'DOWNLOADED' AND mask_status = 'GENERATED'"
    )
