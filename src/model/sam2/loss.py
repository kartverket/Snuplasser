...
class MultiStepMultiMasksAndIous(nn.Module):
    ...
    def _update_losses(
        ...
        probs = torch.sigmoid(src_masks)
        frac = probs.mean(dim=(2, 3))

        if probs.size(1) > 1:
           combo = loss_multimask * self.weight_dict["loss_mask"] \
                  + loss_multidice * self.weight_dict["loss_dice"]
           best_idx = torch.argmin(combo, dim=1)
           batch_idx = torch.arange(combo.size(0), device=combo.device)
           area_loss = frac[batch_idx, best_idx].unsqueeze(1)
        else:
           area_loss = frac

        area_loss = area_loss * target_obj
        losses["loss_area"] = losses.get("loss_area", 0) + area_loss.sum() # For å unngå at den predikerer hele området som snuplass
