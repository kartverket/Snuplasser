...
class PyramidPool(nn.Sequential):
    def __init__(
        self, in_channels, out_channels, pool_size=1, norm_layer=nn.BatchNorm2d
    ):
...
class SharedChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        tailor_sum = 1 / (
            width * height
            + torch.einsum("bnc, bn->bc", K, torch.sum(Q, dim=-2) + self.eps)
        )
...
class DCFAM(nn.Module):
    def __init__(self, encoder_channels=(96, 192, 384, 768), atrous_rates=(6, 12)):
        ...
        self.lf4 = nn.Sequential(
            SeparableConvBNReLU(
                encoder_channels[-1], encoder_channels[-2], dilation=rate_1
            ),
        ...
        self.lf3 = nn.Sequential(
            SeparableConvBNReLU(
                encoder_channels[-2], encoder_channels[-3], dilation=rate_1
            ),
            nn.UpsamplingNearest2d(scale_factor=2),
            SeparableConvBNReLU(
...
def dcswin_base(
    pretrained=True, num_classes=4, weight_path="pretrain_weights/stseg_base.pth"
):
...
def dcswin_small(
    pretrained=True, num_classes=4, weight_path="pretrain_weights/stseg_small.pth"
):
    model = DCSwin(
        encoder_channels=(96, 192, 384, 768),
        num_classes=num_classes,
...
def dcswin_tiny(
    pretrained=True, num_classes=4, weight_path="pretrain_weights/stseg_tiny.pth"
):
    model = DCSwin(
        encoder_channels=(96, 192, 384, 768),
        num_classes=num_classes,
