...
class SnuplassDataset(Dataset):
    def __init__(self, image_dir, mask_dir, dom_dir, split="train", transform=None):
        ...
        if self.split == "train":
            self.file_list = load_numpy_split_stack(image_dir=image_dir, mask_dir=mask_dir, dom_dir=dom_dir)[0]
        else:
            self.file_list = load_numpy_split_stack(image_dir=image_dir, mask_dir=mask_dir, dom_dir=dom_dir)[1]
