...
def _expand_input_conv_weight_to_4_channels(state_dict: dict) -> dict:
    """
    Expands a 3-channel conv weight to 4 channels by repeating the last channel.
    Specifically targets the typical patch_embed.proj.weight.

    If the weight is already 4-channel, it does nothing.

    Raises ValueError if it can't handle the shape.
    """
    ...
    if w.shape[1] == 3:
        # Expand from (out, 3, H, W) -> (out, 4, H, W)
        extra = w[:, -1:, :, :].clone()
        state_dict[key] = torch.cat([w, extra], dim=1)
        print(f"[INFO] Adapted {key} from 3 -> 4 channels at load time")
        return state_dict

    raise ValueError(f"Cannot expand {key}: unexpected shape {w.shape}")
...
def load_checkpoint_and_apply_kernels(
    ...
    # Apply kernels
    pre_train_dict = _expand_input_conv_weight_to_4_channels(pre_train_dict)
    print(f"Loaded checkpoint from {checkpoint_path}")
    logging.debug(
